...
namespace Alteruna
{
	public class Hand : AttributesSync
	{
		...
		[Header("Input")] public InputActionReference PickupAction;

		public float ThrowForceMultiplier = 1000;

		// buffer of all objects inside trigger
		private readonly List<Grabbable> _objectsInTrigger = new();
		...
		private void OnTriggerEnter(Collider other)
		{
			...
			if (other.TryGetComponent(out Grabbable grable)) _objectsInTrigger.Add(grable);
		}

		private void OnTriggerExit(Collider other)
		{
			// remove object as a possible grabbable object.
			if (other.TryGetComponent(out Grabbable grable)) _objectsInTrigger.Remove(grable);
		}
		...
		private void GrabAndSync(InputAction.CallbackContext callbackContext)
		{
			...
			//find closest object
			var minDistance = MAX_GRAB_DISTANCE;
			Grabbable closestObject = null;
			foreach (var grable in _objectsInTrigger)
			{
				var distance = Vector3.Distance(grable.transform.position, transform.position);
				if (distance < minDistance && !grable.Locked)
				{
